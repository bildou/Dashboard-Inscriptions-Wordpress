...
$all_buses = Event_Dashboard_Data::get_all_bus_products();
$current_bus_id = isset($_GET['bus_id']) ? intval($_GET['bus_id']) : 0;
if (empty($current_bus_id) && !empty($all_buses)) {
    $current_bus_id = $all_buses[0]->ID;
}
$stats = Event_Dashboard_Data::get_bus_page_stats($current_bus_id);

// --- 2. LOGIQUE POUR LA TABLE (BLOC QUI MANQUAIT) ---
$per_page = 20;
...
<div class="wrap event-dashboard-container">
    ...
    <div class="dashboard-card kpi-main">
        <?php $current_bus_name = $current_bus_id ? get_the_title($current_bus_id) : 'aucun bus'; ?>
        <h3>Nombre total d'inscrits pour le bus "<?php echo esc_html($current_bus_name); ?>"</h3>
        <p class="kpi-value"><?php echo esc_html($stats['total']); ?></p>
    </div>
    ...
    <div class="dashboard-grid kpi-grid-new" style="grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));">
        ...
        <?php endforeach; else : ?><p>Aucun inscrit pour ce bus.</p><?php endif; ?>
    </div>
    ...
    <nav class="pill-nav">
        ...
        foreach ($all_buses as $bus) : ?>
            <a href="<?php echo esc_url(add_query_arg('bus_id', $bus->ID, $base_nav_url)); ?>" 
               class="<?php echo ($current_bus_id == $bus->ID) ? 'active' : ''; ?>">
    ...
    <div class="dashboard-card full-width">
        <h3>Liste des inscrits du bus</h3>
        <form method="get">
