...
$all_navettes = [
    'bleu'  => 'Navette Parcours Bleu',
    'vert'  => 'Navette Parcours Vert',
];
$current_navette = isset($_GET['navette']) ? sanitize_key($_GET['navette']) : 'bleu';
$stats = Event_Dashboard_Data::get_navette_page_stats($current_navette);

// --- NOUVELLE LOGIQUE POUR LE TEXTE DYNAMIQUE DU KPI ---
$kpi_subtitle = '';
if ($current_navette === 'bleu') {
    $kpi_subtitle = 'pour la navette de XXX vers la pause du midi.';
} elseif ($current_navette === 'vert') {
    $kpi_subtitle = 'pour la navette vers XXX (départ).';
}

// --- 2. LOGIQUE POUR LA TABLE ---
$per_page = 20;
...
$data_args = [
    ...
    'navette_filter' => $current_navette
];
...
<div class="wrap event-dashboard-container">
    ...
    <div class="dashboard-grid kpi-grid-new" style="grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));">
        <?php if (!empty($stats['chapters'])) : foreach ($stats['chapters'] as $chapter_name => $count) : ?>
            <div class="kpi-card-small chapter-kpi"><h4><?php echo esc_html($chapter_name); ?></h4><p><?php echo esc_html($count); ?></p></div>
        <?php endforeach; else : ?><p>Aucun inscrit pour cette navette.</p><?php endif; ?>
    </div>

    <nav class="pill-nav">
        <?php
        $base_nav_url = admin_url('admin.php?page=event-dashboard&subpage=navettes');
        foreach ($all_navettes as $slug => $label) : ?>
            <a href="<?php echo esc_url(add_query_arg('navette', $slug, $base_nav_url)); ?>" 
               class="<?php echo ($current_navette === $slug) ? 'active' : ''; ?>">
    ...
    <div class="dashboard-card full-width">
        <h3>Liste des inscrits pour cette navette</h3>
        <form method="get">
            <input type="hidden" name="page" value="event-dashboard" />
            <input type="hidden" name="subpage" value="navettes" />
            <input type="hidden" name="navette" value="<?php echo esc_attr($current_navette); ?>" />
        ...
        <div class="table-wrapper">
            <table class="custom-data-table">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="<?php echo count($columns); ?>">Aucune inscription trouvée pour cette navette.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
