...
// On récupère le parcours sélectionné dans l'URL, avec 'rouge' comme valeur par défaut.
$current_parcours = isset($_GET['parcours']) ? sanitize_key($_GET['parcours']) : 'rouge';
// On récupère les statistiques pour ce parcours
$stats = Event_Dashboard_Data::get_parcours_page_stats($current_parcours);

// --- 2. LOGIQUE POUR LA TABLE (C'EST ICI QUE SE TROUVAIT L'ERREUR) ---
$per_page = 20;
...
$data_args = [
    ...
    'parcours_filter' => $current_parcours // On passe bien le filtre du parcours courant
];
...
<div class="wrap event-dashboard-container">
    ...
    <div class="dashboard-card kpi-main">
        <h3>Nombre total d'inscrits pour le <?php echo esc_html($all_parcours[$current_parcours]); ?></h3>
    ...
    <div class="dashboard-grid kpi-grid-new" style="grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));">
        ...
        <?php endforeach; else : ?><p>Aucun inscrit pour ce parcours.</p><?php endif; ?>
    </div>
